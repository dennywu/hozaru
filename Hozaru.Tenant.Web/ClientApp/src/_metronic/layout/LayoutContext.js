...
function findAdvancedPageConfig(currentPage, items) {
    if (!items || !Array.isArray(items)) {
        return;
    }
    ...
    for (const item of items) {
        ...
        if (item.page && item.page.includes(":id")) {
            const itemPage = item.page[0] === "/" ? item.page.slice(1) : item.page;
...
function init({ pathname, menuConfig }) {
    ...
    let pageConfig =
        ...
        findAdvancedPageConfig(currentPage, menuConfig.aside.items);
