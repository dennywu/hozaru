...
namespace Hozaru.Core.Configurations
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        {
            return GetSettingValueInternalAsync(name, HozaruSession.TenantId, HozaruSession.UserId);
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesAsync(SettingScopes scopes)
        {
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (!setting.IsInherited &&
                        ((setting.Scopes.HasFlag(SettingScopes.Tenant) && HozaruSession.TenantId.HasValue) || (setting.Scopes.HasFlag(SettingScopes.User) && HozaruSession.UserId.HasValue)))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.Tenant) && HozaruSession.TenantId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForTenantAsync(HozaruSession.TenantId.Value))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
                    ...
                    if (!setting.IsInherited &&
                        (setting.Scopes.HasFlag(SettingScopes.User) && HozaruSession.UserId.HasValue))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.User) && HozaruSession.UserId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForUserAsync(HozaruSession.UserId.Value))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
        ...
        private async Task<SettingInfo> InsertOrUpdateOrDeleteSettingValueAsync(string name, string value, int? tenantId, long? userId)
        {
            ...
            if (settingDefinition.IsInherited)
            {
                ...
                if (userId.HasValue && HozaruSession.TenantId.HasValue)
                {
                    var tenantValue = await GetSettingValueForTenantOrNullAsync(HozaruSession.TenantId.Value, name);
                    if (tenantValue != null)
                    {
