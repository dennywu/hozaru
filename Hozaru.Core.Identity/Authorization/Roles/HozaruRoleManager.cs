...
namespace Hozaru.Core.Identity.Authorization.Roles
{
    ...
    /// </summary>
    public abstract class HozaruRoleManager<TTenant, TRole, TUser>
        ...
        where TUser : HozaruUser<TTenant, TUser>
    {
        ...
        private IRolePermissionStore<TTenant, TRole, TUser> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new HozaruException("Store is not IRolePermissionStore");
                }
        ...
        protected HozaruRoleStore<TTenant, TRole, TUser> HozaruStore { get; private set; }
        ...
        /// Constructor.
        /// </summary>
        protected HozaruRoleManager(
            HozaruRoleStore<TTenant, TRole, TUser> store,
        ...
        public override async Task<IdentityResult> CreateAsync(TRole role)
        {
            ...
            {
                role.TenantId = HozaruSession.TenantId.Value;
            }
        ...
        /// <exception cref="HozaruException">Throws exception if no role with given id</exception>
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            var role = await FindByIdAsync(roleId);
            if (role == null)
            {
                throw new HozaruException("There is no role with id: " + roleId);
            }
        ...
        /// <param name="roleName">Role name</param>
        /// <returns>Role</returns>
        /// <exception cref="HozaruException">Throws exception if no role with given roleName</exception>
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            var role = await FindByNameAsync(roleName);
            if (role == null)
            {
                throw new HozaruException("There is no role with name: " + roleName);
            }
        ...
        {
            return HozaruStore.FindByDisplayNameAsync(displayName);
        }
